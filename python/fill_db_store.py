...
def create_artist(artist_name):
    ...
    insert_query = "INSERT INTO Artist (Name, Country) VALUES (?, ?)"
    cursor.execute(insert_query, artist_name, new_artist_country)
    connSql.commit()
    # Obtener el último ID insertado
    cursor.execute("SELECT @@IDENTITY  AS NewArtistID")
...
def create_genre(genre_name):
    ...
    connSql.commit()
    # Obtener el último ID insertado
    cursor.execute("SELECT @@IDENTITY  AS NewGenreID")
...
def create_format(format_name):
    ...
    cursor = connSql.cursor()        
    # Consulta para insertar el nuevo artista
    insert_query = "INSERT INTO Format (Name) VALUES (?)"
    cursor.execute(insert_query, format_name)
    connSql.commit()
    # Obtener el último ID insertado
    cursor.execute("SELECT @@IDENTITY  AS NewFormatID")
