...
def get_id_artist(artist_df, artist_text):
    if artist_df['Name'].isin([artist_text]).any():
        return artist_df.loc[artist_df['Name'] == artist_text, 'Id'].values[0]
    else:        
        return create_artist(artist_text)
...
def get_id_genre(genre_df, genre_text):
    if genre_df['Name'].isin([genre_text]).any():
        return genre_df.loc[genre_df['Name'] == genre_text, 'Id'].values[0]
    else:        
        return create_genre(genre_text)
...
def get_id_format(format_df, format_text):
    if format_df['Name'].isin([format_text]).any():
        return format_df.loc[format_df['Name'] == format_text, 'Id'].values[0]
    else:        
        return create_format(format_text)
...
try:
    ...
    for index, col in data_excel.iterrows():
        ...
        country = col["Country"]
        Year_Db = col["Year"]
        StatusCover = col["StatusCover"]
        StatusGeneral = col["StatusGeneral"]
        Price = col["Price"]
        Matrix = col["Matrix"]
        Label = col["Label"]
